...
export default function logEvent(eventAction: LogEvent, logData?: LogData): void {
  ...
  const traderSettingsStr = window.localStorage.getItem(LOCAL_STORAGE_TRADER_SETTINGS_KEY)
  ...
  // Insert query params
  const queryParams = Array.from(new URLSearchParams(window.location.search).entries()).reduce(
    (queries, [key, val]) => {
      if (val === undefined) {
        return queries
      } else {
