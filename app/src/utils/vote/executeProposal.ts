...
const executeProposal = async (account: string, proposalId: number): Promise<PopulatedTransaction> => {
  const lyraGovernanceV2Contract = getContract(ContractId.LyraGovernanceV2, AppNetwork.Ethereum)
  const data = lyraGovernanceV2Contract.interface.encodeFunctionData('execute', [proposalId])
  const tx = buildTx(
    mainnetProvider,
