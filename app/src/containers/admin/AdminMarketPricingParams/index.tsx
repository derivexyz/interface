...
import { AdminPricingParams, Market } from '@lyrafinance/lyra-js'
...
const zeroDecimalKeys: (keyof AdminPricingParams)[] = [
  'spotPriceFee1xPoint',
  'spotPriceFee2xPoint',
...
const AdminMarketPricingParams = ({ market, isExpanded, onClickExpand }: Props) => {
  const admin = useAdmin(market.lyra.network)
  const [isConfirmOpen, setIsConfirmOpen] = useState(false)
  const [params, setParams] = useState<Partial<AdminPricingParams>>({})
  const [newParams, setNewParams] = useState<Partial<AdminPricingParams>>({})
  ...
  return (
    <>
      ...
      >
        <Box p={4}>
          {Object.entries(market.__data.marketParameters.pricingParams).map(([key, value]) => {
            if (parseInt(key) || parseInt(key) === 0) {
              return
            }
            const typedKey = key as keyof AdminPricingParams
          ...
          <Button
            ...
            label="Update"
            onClick={async () => {
              const { tx, params: newParams } = await admin.setPricingParams(market.address, params)
