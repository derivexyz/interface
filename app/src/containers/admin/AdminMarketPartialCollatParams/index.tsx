...
import { AdminPartialCollatParams, Market } from '@lyrafinance/lyra-js'
...
const zeroDecimalKeys: (keyof AdminPartialCollatParams)[] = []

const AdminMarketPartialCollatParams = ({ market, isExpanded, onClickExpand }: Props) => {
  const admin = useAdmin(market.lyra.network)
  const [isConfirmOpen, setIsConfirmOpen] = useState(false)
  const [params, setParams] = useState<Partial<AdminPartialCollatParams>>({})
  const [newParams, setNewParams] = useState<Partial<AdminPartialCollatParams>>({})
  ...
  return (
    <>
      ...
      >
        <Box p={4}>
          {Object.entries(market.__data.marketParameters.partialCollatParams).map(([key, value]) => {
            if (parseInt(key) || parseInt(key) === 0) {
              return
            }
            const typedKey = key as keyof AdminPartialCollatParams
          ...
          <Button
            ...
            width={200}
            onClick={async () => {
              const { tx, params: newParams } = await admin.setPartialCollatParams(market.address, params)
