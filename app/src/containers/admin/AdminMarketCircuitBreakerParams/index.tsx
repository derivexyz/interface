...
import { AdminCircuitBreakerParams, Market } from '@lyrafinance/lyra-js'
...
const zeroDecimalKeys: (keyof AdminCircuitBreakerParams)[] = [
  'boardSettlementCBTimeout',
  'contractAdjustmentCBTimeout',
  'ivVarianceCBTimeout',
  'liquidityCBTimeout',
  'skewVarianceCBTimeout',
]

const AdminMarketCircuitBreakerParams = ({ market, isExpanded, onClickExpand }: Props) => {
  const admin = useAdmin(market.lyra.network)
  const [isConfirmOpen, setIsConfirmOpen] = useState(false)
  const [params, setParams] = useState<Partial<AdminCircuitBreakerParams>>({})
  const [newParams, setNewParams] = useState<Partial<AdminCircuitBreakerParams>>({})
  ...
  if (!('cbParams' in market.__data.marketParameters)) {
    return null
  }
  ...
  return (
    <>
      ...
      >
        <Box p={4}>
          {Object.entries(market.__data.marketParameters.cbParams).map(([key, value]) => {
            if (parseInt(key) || parseInt(key) === 0) {
              return
            }
            const typedKey = key as keyof AdminCircuitBreakerParams
          ...
          <Button
            ...
            label="Update"
            onClick={async () => {
              const { tx, params: newParams } = await admin.setCircuitBreakerParams(market.address, params)
      ...
      <Modal isOpen={isConfirmOpen} onClose={() => setIsConfirmOpen(false)} title="Confirm New Circuit Breaker Params">
        <CardBody>
          <Box>
            {newParams
...
export default AdminMarketCircuitBreakerParams
