...
export const useCoolMode = (imageUrls: string[]) => {
  const ref = useRef<HTMLElement>(null)

  useEffect(() => {
    if (ref.current && imageUrls.length) {
      return makeElementCool(ref.current, imageUrls)
    }
  }, [imageUrls])
...
function makeElementCool(element: HTMLElement, imageUrls: string[]): () => void {
  instanceCounter++
