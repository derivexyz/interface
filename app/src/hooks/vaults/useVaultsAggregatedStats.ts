...
const fetcher = async (period: number): Promise<AggregateVaultStats> => {
...
export default function useVaultsAggregatedStats(period: number): AggregateVaultStats | null {
  const [vaultsStats] = useFetch(FetchId.VaultAggregateStats, [period], fetcher)
  return vaultsStats
}
