...
const getAdminBoardPath = (args: PageArgsMap[PageId.AdminBoard]): string => {
  return `/admin/board?market=${args.marketAddressOrName}&id=${args.boardId}`.toLowerCase()
}
...
const getPositionPath = (args: PageArgsMap[PageId.Position]): string => {
  return `/position?market=${args.marketAddressOrName}&id=${args.positionId}`.toLowerCase()
}
