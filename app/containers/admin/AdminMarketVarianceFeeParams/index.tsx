...
import { Market, VarianceFeeParams } from '@lyrafinance/lyra-js'
...
const zeroDecimalKeys: (keyof VarianceFeeParams)[] = []
...
const AdminMarketVarianceFeeParams = withSuspense(
  ({ market, isExpanded, onClickExpand, onParamUpdate }: Props) => {
    ...
    const [newParams, setNewParams] = useState<Partial<VarianceFeeParams>>({})
    ...
    return (
      <>
        <Box my={2}>
          ...
          >
            <Box px={4}>
              {Object.entries(market.marketParameters.varianceFeeParams).map(([key, value]) => {
                if (parseInt(key) || parseInt(key) === 0) {
                  return
                }
                const typedKey = key as keyof VarianceFeeParams
        ...
        <Modal
          ...
          title="Confirm New Variance Fee Parameters"
        >
          <CardBody>
            <Box>
              {newParams
